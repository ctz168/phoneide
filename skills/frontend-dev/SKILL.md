...
name: frontend-dev
description: |
  Frontend Studio — full-stack frontend development combining premium UI design, cinematic animations,
...
Generate all image/video/audio assets. NEVER use placeholder URLs (unsplash, picsum, placeholder.com, via.placeholder, placehold.co, etc.) or external URLs.

1. Parse asset requirements (type, style, spec, usage)
2. Craft optimized prompts, show to user, confirm before generating
3. Execute via scripts or CLI tools, save to project — do NOT proceed to Phase 5 until all assets are saved locally
...
In the myagent environment, use the following methods for asset generation:

### Image Generation
Use the `z-ai-generate` CLI tool or `z-ai-web-dev-sdk`:

```bash
# CLI tool (recommended for quick generation)
z-ai-generate --prompt "A cat playing in the garden" --output "./hero-bg.png" --size 1024x1024

# Available sizes: 1024x1024, 768x1344, 864x1152, 1344x768, 1152x864, 1440x720, 720x1440
```

Or use the MiniMax scripts (requires `MINIMAX_API_KEY` and `MINIMAX_API_BASE`):
```bash
python scripts/minimax_image.py "A beautiful landscape" -o hero-bg.png --ratio 16:9
```

### Video Generation
Use MiniMax scripts (requires `MINIMAX_API_KEY` and `MINIMAX_API_BASE`):
```bash
python scripts/minimax_video.py "Ocean waves [Truck left]" -o hero-video.mp4 --resolution 768P
```

### Text-to-Speech
Use MiniMax scripts (requires `MINIMAX_API_KEY` and `MINIMAX_API_BASE`):
```bash
python scripts/minimax_tts.py "Welcome to our app" -o intro.mp3 -v female-shaonv
```

### Music Generation
Use MiniMax scripts (requires `MINIMAX_API_KEY` and `MINIMAX_API_BASE`):
```bash
python scripts/minimax_music.py --prompt "Upbeat pop, energetic" --instrumental -o bgm.mp3
```
